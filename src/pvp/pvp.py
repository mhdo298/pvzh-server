...
pvp = Blueprint('pvp', __name__, url_prefix='/<string:username>/<string:repo>/pvp')
...
def player_pvp_data(username, repo, version):
    return {
        "gamesPlayed": 0,
        "stars": 0,
        "id": get_id(),
...
async def init_game(username, repo, version):
    pid = get_id()
    data = await request.get_json()
    gi = data['gi']
    mod_name = f'{username}+{repo}'

    seed1 = await request.redis.blmove(f'{mod_name}-{pid}-matchinfo', f'{mod_name}-{gi}-matchinfo', 5)
    if seed1 is None:
        return {}
    data1 = await request.redis.blmove(f'{mod_name}-{pid}-matchinfo', f'{mod_name}-{gi}-matchinfo', 5)
    if data1 is None:
        return {}
    seed2 = await request.redis.blpop(f'{mod_name}-{pid}-matchinfo', 5)
    if seed2 is None:
        return {}
    seed2 = seed2[1]
    data2 = await request.redis.blpop(f'{mod_name}-{pid}-matchinfo', 5)
    if data2 is None:
        return {}
    data2 = data2[1]
    seed = int(seed1) ^ int(seed2)
    model = make_entity_model(json.loads(data1), json.loads(data2))
    print(model)
    return {
        "entityModel": model,
...
async def pvp_send_update(username, repo, version):
    data = await request.get_json()
    gi = data['gi']
    mod_name = f'{username}+{repo}'
    await request.redis.rpush(f'{mod_name}-{gi}-update', json.dumps(data))
    await request.redis.expire(f'{mod_name}-{gi}-update', 1800)
    return {
        "ty": "PlayResponse",
        "p": "Play"
    }
...
async def pvp_poll(username, repo, version):
    pid = get_id()
    data = await request.get_json()
    l = int(data['l'])
    mod_name = f'{username}+{repo}'
    update = await request.redis.blpop([f'{mod_name}-{pid}-update'], 4)
    if update:
        return {
            "m": [update[1].decode()],
            "l": l + 1,
